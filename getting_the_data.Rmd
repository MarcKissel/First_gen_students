...
data_all %>% filter(!is.na(value)) %>% select(-field1) %>% 
  ...
  ggplot(aes(x=parents_data2, y = median, fill = parents_data2)) + geom_col() + facet_wrap(~year, scales = "free_y") + coord_flip() + scale_y_continuous(labels= scales::percent_format())
...
data_all %>% filter(!is.na(value)) %>% select(-field1) %>% 
  ...
  filter(is.na(parents_data1),
         parents_data2 != "Total number") %>% 
    mutate(median = median(value)) %>% 
  ggplot(aes(x=parents_data2, y = median, fill = parents_data2)) + geom_col() + facet_wrap(~year, scales = "free_y") + coord_flip() + scale_y_continuous(labels= scales::percent_format()) + labs(title = "mom")
...
2013	Advanced degree	23.60
2013	Bachelor's degree	27.20
2013	High school or less	32.00
2013	Some collegea  15.9
